...
19. `sql/migrate-product-sales-disabled-server.sql` — coloana `products.sales_disabled` (butonul „Indisponibilizează vânzarea” din editorul de produs și din meniul „⋯” al listei: ascuns din catalog, refuzat la checkout, stoc 0 în feed-ul Facebook, în schema Google și în BaseLinker) + corecția stocului pentru retururile încă pe drum. De la această versiune: stocul unui retur revine abia când coletul e marcat reîntors (`return_received`), comenzile noi / confirmate încă neambalate rezervă stocul (magazinul și feed-ul arată stocul vandabil), iar sincronizarea SmartBill pune pe 0 codurile deja facturate care lipsesc din lista de stocuri SmartBill.
18. `sql/migrate-courier-rates-server.sql` — tarifele din contracte (Fan Courier 2730/23.06.2026, anexa DPD 5.001–10.000 exp./lună) salvate în `routing_settings` (`rates_fan-courier`, `rates_dpd`), fără TVA. Indexul de combustibil și taxa de forță de muncă se completează din admin → Curieri → Tarife curieri (sau rămân cele din `config.php`). Aceleași valori sunt și implicite în cod, deci migrarea doar le face vizibile ca „editate în admin”.
17. `sql/migrate-courier-shipments-server.sql` — `courier_shipments`, `courier_settings`, `routing_settings` (panoul Curieri: tracking real, la timp, retururi; etapa 1 din alocarea automată). Sincronizarea rulează automat din admin la 30 min; opțional cron cPanel: `courier_shipments.php?cron=1&secret=...` cu `courier_sync.secret` din `config.php`. Tarife Fan în `config.php` → `fan.contract_rates` (vezi `config.example.php`).
    - Panoul curierat (`/admin/curieri`: livrare în 24 h, la timp pe județe, retururi per curier), „Rutare curieri” (ponderi, curieri, simulator, real vs. estimare) și „Tarife curieri” (contract vs. cost real, editabile din admin) rulează din `courier_routing.php` peste aceleași tabele — fără SQL suplimentar. Tarifele salvate din admin se țin în `routing_settings` (`rates_<curier>`) și au prioritate față de `config.php`.
